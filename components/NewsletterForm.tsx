...
export default function NewsletterForm({ variant = 'footer' }: { variant?: 'footer' | 'promo' }) {
    ...
    return (
      <form onSubmit={subscribe} className="w-full max-w-xl">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-2">
          <input id="newsletter-promo-email" value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" inputMode="email" required placeholder="Enter your email address" className="h-12 w-full min-w-0 flex-1 rounded-lg border border-white/20 bg-white px-4 text-base text-[#071225] outline-none transition focus:border-gtred focus:ring-2 focus:ring-gtred/30 sm:text-sm" />
  ...
  return (
    <div>
      ...
      <form onSubmit={subscribe} className="flex flex-col gap-3 sm:flex-row sm:gap-2">
        <label className="sr-only" htmlFor="newsletter-footer-email">Email address</label>
        <input id="newsletter-footer-email" value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" inputMode="email" required placeholder="Email address" className="h-12 w-full min-w-0 flex-1 rounded-lg border border-white/10 bg-white/[0.08] px-4 text-base text-white placeholder:text-white/35 outline-none transition focus:border-gtred focus:ring-2 focus:ring-gtred/30 sm:text-sm" />
